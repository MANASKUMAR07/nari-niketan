...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        # Running Header
        self.drawString(36, 11 * inch - 28, "NARI NIKETAN — Complete Technology, System Architecture & Implementation Documentation")
        ...
        self.drawString(36, 22, "CONFIDENTIAL & PROPRIETARY — Official Technical Audit & Architectural Reference Manual")
...
def build_master_pdf():
    ...
    # Brand Colors
    c_primary = colors.HexColor("#8B1A4A")     # Royal Maroon
    c_gold = colors.HexColor("#B8860B")        # Regal Gold
    c_dark = colors.HexColor("#111827")        # Charcoal
    c_body = colors.HexColor("#374151")        # Text Body
    c_dim = colors.HexColor("#6B7280")         # Dim Text
    c_code = colors.HexColor("#065F46")        # Forest Green
    c_table_bg = colors.HexColor("#F9FAFB")    # Off-white
    ...
    def hr():
        return HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#E5E7EB"), spaceBefore=4, spaceAfter=6)
    ...
    def make_table(data, col_widths, is_header=True):
        ...
        style = [
            ...
            ('PADDING', (0, 0), (-1, -1), 3.5),
            ('ROWBACKGROUNDS', (0, 1 if is_header else 0), (-1, -1), [colors.white, c_table_bg]),
        ]
        if is_header:
    ...
    meta_text = """
    <b>Complete Technical Audit, System Architecture &amp; Developer Reference Manual</b><br/>
    <b>Version:</b> 2.0.0 (Production Release) &nbsp;|&nbsp; <b>Release Date:</b> September 2026<br/>
    <b>Platform:</b> High-Performance Omnichannel Indian Ethnic E-Commerce Platform<br/>
    <b>Primary Domain:</b> https://www.nariniketan.shop &nbsp;|&nbsp; <b>Firebase Hosting:</b> nari-niketan<br/>
    <b>Target Audience:</b> Software Engineers, System Architects, Security Evaluators, B.Tech Students &amp; Lead Maintainers
    """
    ...
    cover_summary_data = [
        [Paragraph("<b>Audit Dimension / Category</b>", table_header), Paragraph("<b>Audited State &amp; Verification Details</b>", table_header)],
        [Paragraph("<b>Architecture Pattern</b>", table_cell_bold), Paragraph("Serverless Jamstack Architecture + Client-Side Distributed AI Suite", table_cell)],
        [Paragraph("<b>Frontend Core</b>", table_cell_bold), Paragraph("Vanilla JavaScript ES6+, Semantic HTML5, Modular CSS3 (Zero Framework Overhead)", table_cell)],
        [Paragraph("<b>Database Layer</b>", table_cell_bold), Paragraph("Google Cloud Firestore (17 Collections) with Disk-Cache Offline Persistence", table_cell)],
        [Paragraph("<b>Storage Layer</b>", table_cell_bold), Paragraph("Firebase Cloud Storage (max 6 images/product, 5MB limit, image/* MIME enforcement)", table_cell)],
        [Paragraph("<b>Authentication &amp; RBAC</b>", table_cell_bold), Paragraph("Firebase Auth (Email/Pass, Google OAuth, Phone OTP) + 4-Tier Security (Customer/Seller/Admin/Owner)", table_cell)],
        [Paragraph("<b>Internal AI Suite</b>", table_cell_bold), Paragraph("Nari AI Stylist RAG, NLP Search, Virtual Try-On Canvas, Nari AI Ads, Seller Photo Wizard", table_cell)],
        [Paragraph("<b>Continuous Operations QA</b>", table_cell_bold), Paragraph("Nari AI Website Operations Center (12 Pillars, 8 Viewports Sandbox DOM Testing)", table_cell)],
        [Paragraph("<b>Fulfilment Pipeline</b>", table_cell_bold), Paragraph("Dual Mode: Direct Home Delivery + Physical Store Pickup (Google Maps Geolocation)", table_cell)],
        [Paragraph("<b>Edge Infrastructure</b>", table_cell_bold), Paragraph("Firebase Hosting backed by Fastly Global Edge CDN, TLS 1.3, Strict HTTP Headers", table_cell)],
        [Paragraph("<b>Verification Summary</b>", table_cell_bold), Paragraph("26/26 Automated Tests Passing (scripts/nari-ops-audit.js) — 100% Production Ready", table_cell)]
    ]
    story.append(make_table(cover_summary_data, [190, 350]))
    story.append(Spacer(1, 20))
    story.append(p("<b>Classification:</b> CONFIDENTIAL / PROPRIETARY SYSTEM ARCHITECTURE SPECIFICATION", ParagraphStyle('Class', parent=body_style, alignment=1, fontSize=7.5, textColor=c_gold)))
    story.append(PageBreak())

    # =========================================================================
    # TABLE OF CONTENTS
    # =========================================================================
    story.append(p("Comprehensive Table of Contents (43 Parts)", h1_style))
    story.append(hr())
    toc_data = [
        ["Part 1: Executive Summary & Overview", "Part 23: Backend & Data Access Layer Architecture"],
        ...
        ["Part 6: Customer Storefront Architecture", "Part 28: SEO & Structured Data Architecture"],
        ...
        ["Part 19: Product Media & Image Storage Pipeline", "Part 41: Master System Architecture Blueprint"],
    ...
    story.append(hr())
    story.append(p("<b>1.1 Purpose of Nari Niketan:</b><br/>Nari Niketan is an enterprise e-commerce platform dedicated to handcrafted Indian ethnic fashion, including Sarees (Banarasi silk, Kanjivaram, Chanderi, Georgette), Bridal Lehengas, Salwar Suits, Kurtas, Dupattas, and Jewelry Accessories. It eliminates intermediaries, connecting regional weavers and boutique sellers directly to consumers across India."))
    story.append(p("<b>1.2 Problem Solved:</b><br/>Traditional ethnic fashion e-commerce suffers from heavy client bundles, horizontal visual overflows on compact mobile viewports (320px–375px), lack of intelligent ethnic styling assistance, and inflexible fulfillment. Nari Niketan solves this via a lightweight serverless Jamstack core, integrated client-side AI stylist and search engines, multi-viewport layout sentinels, and hybrid Home Delivery / Store Pickup fulfillment."))
    story.append(p("<b>1.3 Major User Roles:</b>"))
    story.append(p("• <b>Customer:</b> Discovers products, uses natural language search and virtual try-on, consults AI Stylist, places orders via Home Delivery or Store Pickup, and tracks real-time shipment.", bullet_style))
    story.append(p("• <b>Seller:</b> Approved merchants who manage inventory, upload product images (max 6), generate AI descriptions across 7 brand tones, and process customer orders.", bullet_style))
    story.append(p("• <b>Admin:</b> Store managers who oversee product approvals, commissions, marketing campaigns, GST invoices, audit logs, and website health.", bullet_style))
    ...
    story.append(p("Part 2 — Complete Technology Stack (Confirmed Matrix)", h1_style))
    story.append(hr())
    story.append(p("Every technology item below has been audited and classified strictly as <b>CONFIRMED</b>, <b>INFERRED</b>, or <b>NOT FOUND</b>."))
    ...
    story.append(make_table(tech_table_data, [65, 105, 60, 75, 235]))
...
└── sw.js                           # PWA Service Worker (stale-while-revalidate)"""
    ...
    # =========================================================================
    # PART 4 & 5: SYSTEM ARCHITECTURE
    # =========================================================================
    story.append(Spacer(1, 8))
    story.append(p("Part 4 & 5 — System Architecture & Layering Model", h1_style))
    story.append(hr())
    ...
    story.append(make_table(db_schema_data, [75, 235, 95, 135]))
    ...
    story.append(make_table(rbac_data, [170, 75, 80, 95, 120]))
    ...
    story.append(make_table(ops_pillars, [95, 255, 190]))
    ...
    story.append(make_table(threat_data, [85, 105, 50, 225, 75]))
    ...
    # =========================================================================
    # PART 41: MASTER ARCHITECTURE BLUEPRINT
    # =========================================================================
    story.append(PageBreak())
    ...
    story.append(make_table(chk_data, [120, 90, 75, 255]))
    ...
    sys.stdout.write(f"PDF generated successfully at: {pdf_filename}\n")
